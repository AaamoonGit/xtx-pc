...
import { getOrderListRequest } from '@/apis/order'

const getOrderList = async () => {
  const res = await getOrderListRequest()
  console.log(res)
  checkInfo.value = res.result
  defaultAddress.value = checkInfo.value.userAddresses.find(
...
const activeAddr = ref(null)
const isActive = (addr) => {
  activeAddr.value = addr
}
...
const confirm = () => {
  ...
  defaultAddress.value = activeAddr.value
}

import { createOrderRequest } from '@/apis/order'
const orderInfo = ref({})
...
const createOrder = async () => {
  ...
  router.push(`/pay/${orderInfo.value.id}`)
}
</script>
...
<template>
        ...
        <div class="box-body">
          <div class="address">
            ...
            <div class="action">
              <el-button
                size="large"
                @click="((toggleFlag = true), (activeId = 0))"
                >切换地址</el-button
              >
  ...
  <el-dialog v-model="toggleFlag" title="切换收货地址" width="30%" center>
    <div class="addressWrapper">
      <div
        class="text item"
        :class="{ active: item.id === activeAddr?.id }"
        @click="isActive(item)"
